...
// 订单状态
var orderStatusObj = {1:'等待承兑',2:'承兑中',3:'承兑完成',4:'取消承兑',5:"承兑失败"};
    ...
    UserTrade.prototype = {
        ...
        init: function () {
            ...
            var option = {
                ...
                ajax: {
                    url: urlPath + '/orderAccept/dataList',
                    data: function (d) {
                        //添加额外的数据请求参数
                        d.acceptNo = curr.$acceptNo.val();
                ...
                columns: [
                    {},
                    {data: 'acceptNo', name: 'acceptNo'},
                    {data: 'consignee', name: 'consignee'},
                    {data: 'consigneeMobile', name: 'consigneeMobile'},
...
function look(op){
    ...
    $.ajax({
        url: urlPath+'/orderAccept/selectOrder',
        type: 'POST',
        dataType:'json',
        data:{id:id},
        success: function (data) {
            if(data.status=='success'){
                ...
                form.find("input[name='acceptNo']").val(resultData.acceptNo);
                ...
                form.find("input[name='consignee']").val(resultData.consignee);
                form.find("input[name='consigneeMobile']").val(resultData.consigneeMobile);
                form.find("textarea[name='consigneeAddress']").text(resultData.consigneeAddress);
