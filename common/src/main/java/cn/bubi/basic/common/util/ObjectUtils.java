...
/**
 * @Author: JoinHan
 * @Date: Created in 11:25 2018/2/7
 * @Modified By：
 */
public class ObjectUtils {
     ...
     */
    public static Map<String, Object> objectToMap(Object obj, boolean recursion, Map<String, Object> map)
            throws IllegalAccessException {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (value != null) {
                ...
                if (isBaseDataType(valueClass)) {// 判断是不是基本类型
                    map.put(fieldName, value);
                } else {
                    if (List.class.isAssignableFrom(valueClass)) { // list 特殊处理
                    ...
                    if (Set.class.isAssignableFrom(valueClass)) { // Set 特殊处理
                        map.put(field.getName(), value);
                    }
                    if (Enum.class.isAssignableFrom(valueClass)) { // Enum 特殊处理
                        map.put(field.getName(), value);
                        continue;
                    }

                    if (recursion) {
                        objectToMap(value, recursion, map);
                    }
                }
            }
        }
